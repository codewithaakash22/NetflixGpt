...
const VideoTitle = ({title, overview,movieId}) => {
  ...
  return (
    <div className="w-screen pt-[35%] md:p-[25%] px-6 md:px-16 absolute bg-gradient-to-tr from-black/80 text-white">
      <h2 className=" md:text-4xl text-2xl font-bold">{title}</h2>
      <p className="hidden md:block md:w-4/12 mt-4 text-justify">{overview}</p>
      <div className="my-4">
        <button className="bg-white text-black font-bold w-20 p-1 md:w-28  md:p-2 rounded-lg md:mr-2 hover:opacity-80"><Link to={'/movie/' + movieId}><i className="fa-solid fa-play"></i> Play</Link></button>
